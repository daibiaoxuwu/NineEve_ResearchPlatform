...
export default {
  ...
  methods: {
    onRegister() {
      ...
      if (that.registerAgreement == true) {
        ...
        if (that.registerPassword == that.registerPasswordRepetition) {
          ...
          alert(
            "The password repetition is not correct.\n 需要输入一致的密码."
          );
        }
      } else {
        alert(
          "You should agree with Term and Conditions of the service first!\n请点击 同意 勾选框. "
        );
      }
    },
    ...
    getEmailCaptcha() {
      ...
      $.get("/register/getCaptcha", { email: that.registerEmail }).then(
        function(data) {
          alert(
            "A CAPTCHA has been sent to your email, please input it.\n 验证码已发送至您的邮箱, 请输入您收到的验证码."
          );
